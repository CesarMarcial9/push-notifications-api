...
const result = dotenv.config({path: `${process.cwd()}/.env.${process.env.NODE_ENV}`})

// check if dotenv could not be configured correctly
if (result.error) {
  throw result.error
}
/**
...
const SendNotification = async (completeInsight) => {
  ...
  notification.include_external_user_ids = [insight.user_id];
